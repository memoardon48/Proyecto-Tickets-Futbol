...
<head>
    ...
    <link rel="preload" href="css/ligaSpain.css" as="style">
    <link rel="stylesheet" href="css/ligaSpain.css">
...
<body>
    ...
    <main>
        <div class="contenedorTop">
            <section class="banner coll-12 coll-s-12">
                <div class="cajaBanner coll-6 coll-s-7">
                    <a href="inicio.php">Inicio </a>/ Liga Española <!-- ------------------- aqui  -->           
                    <h1>Tickets LaLiga</h1>
                    <p>Tickets para tus equipos españoles favoritos.</p><!-- ------------------- aqui  -->      
        ...
        <section class="areaTickets coll-12 coll-s-12">
            ...
            <?php
                $sentencia = $pdo->prepare("SELECT p.id, fecha_hora, e.nombre as estadio, e.ciudad, temporada, jornada, l.nombre as liga FROM partido as p INNER JOIN estadio e ON p.id_estadio = e.id INNER JOIN liga l ON p.id_liga = l.id WHERE p.id_liga=2");
                $sentencia->execute();
